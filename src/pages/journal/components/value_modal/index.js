...
import { Switch, Select, Button, MenuItem, IconButton, ToggleButtonGroup, ToggleButton, ThemeProvider, TextField } from "@mui/material";
...
function ValueModal(props){
	...
	const date = (type) => {
		...
		if(student){
			...
			if(student.days && props.day !== null){
				...
				if( type === "values")
					return student.days[props.day].values;
			}
		}
			...
			return vals.map((value, index) => (
				<div className="value-wrapper" key={index}>
					<div className="value-type" data-val={index}>
						<Select onChange={ handleVlaueType } label="Тип оценки" variant="standard" value={value.type} sx={{width: '100%'}}>
							<MenuItem data-val={index} value="Тестирование">Тестирование</MenuItem>
							<MenuItem data-val={index} value="Лабораторная работа">Лабораторная работа</MenuItem>
							<MenuItem data-val={index} value="Контрольная работа">Контрольная работа</MenuItem>
							<MenuItem data-val={index} value="Расчётно-графическая работа">Расчётно-графическая работа</MenuItem>
							<MenuItem data-val={index} value="Домашняя работа">Домашняя работа</MenuItem>
							<MenuItem data-val={index} value="Другое">Другое</MenuItem>
						</Select>
					</div>
					<div className="value">
						<ThemeProvider theme={toggleTheme}>
							...
							>
								<ToggleButton size="small" value={5} data-value={index} sx={{flexGrow: 1}}>
								5
								</ToggleButton>
								<ToggleButton size="small" value={4} data-value={index} sx={{flexGrow: 1}}>
								4
								</ToggleButton>
								<ToggleButton size="small" value={3} data-value={index} sx={{flexGrow: 1}}>
								3
								</ToggleButton>
								<ToggleButton size="small" value={2} data-value={index} sx={{flexGrow: 1}}>
								Неуд
								</ToggleButton>
							</ToggleButtonGroup>
						</ThemeProvider>
	...
	const inputLabel = () => {
		let clearFileClass = student.days[props.day].absent_link === null || student.days[props.day].absent_link === "" ? "clear-file hide" : "clear-file"
		return(
			<label htmlFor="reason_file" className="file-wrapper">
				<div className="file-label">
					<AttachmentRounded />
					<span>{ student.days[props.day].absent_link || "Выберите файл" }</span>
				</div>
				<div className="clear-wrapper">
					<IconButton onClick={clearFile} className={clearFileClass}><DeleteRounded /></IconButton>
				</div>
			</label>
		)
	}
			...
			return(
				<div className={reasonClass}>
					<div className="reason-block">
						<TextField sx={{width: '100%'}} onChange={ docnameChange } variant="standard" value={student.days[props.day].absent_doc} label="Название документа" />
					</div>
					<div className="reason-block">
						<input type="file" id="reason_file" style={{display: 'none'}} onChange={handleFile} />
						{ inputLabel() }
					</div>
					<div className="reason-block">
						<TextField sx={{width: '100%'}} onChange={ commentChange } multiline={true} variant="standard" value={student.days[props.day].absent_reason || ""} label="Комментарий" />
					</div>
				</div>
			)
		}
	...
	return(
		<div className={props.open ? "modal-wrapper open" : "modal-wrapper"} id="value-modal">
			<div className="modal">
				<div className="modal-content">
					...
					<div className="kv-pair">
						<div className="key">Преподаватель</div>
						<div className="value">{student?.teacher}</div>
					</div>
					<div className="kv-pair">
						<div className="key">Присутствие</div>
						<div className="value">
				...
				<div className="modal-footer">
					<Button sx={{color: 'red'}} variant="text" onClick={clearValues}>Очистить</Button>
					<Button variant="text" onClick={saveStudent}>Сохранить</Button>
				</div>
			</div>
		</div>
	)
